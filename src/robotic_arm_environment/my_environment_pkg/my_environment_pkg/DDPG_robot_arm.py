...
class OUNoise(object):
    ...
    def __init__(self, low,high, mu=0.0, theta=0.15, max_sigma=0.3, min_sigma=0.3, decay_period=100000):
        self.mu = mu
        self.theta = theta
...
class DDPGagent:
    ...
    def __init__(self, hidden_size=256, actor_learning_rate=1e-4, critic_learning_rate=1e-3,
                       gamma=0.99, tau=1e-2, max_memory_size=50000):

        self.num_states = 12  # 3
        self.num_actions = 6    # 1
...
def main(args=None):
    ...
    for episode in range (EPISODE):
        ...
        for step in range (20):
            ...
            new_state  = run_env_node.state_space_funct()
        ...
        if episode % 100 == 0 or episode == EPISODE - 1:
            latest_actor_fname = 'checkpoint_actor' + str(episode) + '.pth'
            latest_critic_fname = 'checkpoint_critic' + str(episode) + '.pth'
            torch.save(agent.actor.state_dict(), latest_actor_fname)
            torch.save(agent.critic.state_dict(), latest_critic_fname)
    plt.plot(rewards)
    plt.plot(avg_rewards)
    plt.plot()
    plt.xlabel('Episode')
