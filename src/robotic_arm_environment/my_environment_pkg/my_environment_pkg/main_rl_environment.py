...
class MyRLEnvironmentNode(Node):
	...
	def __init__ (self):
		...
		self.joint_state_subscription =self.create_subscription(JointState, '/joint_states',self.initial_joint,20)
		
		# Subcribe topic with the sphere position
		self.target_point_subscription =self.create_subscription(ModelStates, '/gazebo/model_states',self.initial_target,20)

		# # Subcribe topic with the joints states
		# self.joint_state_subscription = message_filters.Subscriber(self, JointState, '/joint_states',self.initial_joint)
		
		# # Subcribe topic with the sphere position
		# self.target_point_subscription = message_filters.Subscriber(self, ModelStates, '/gazebo/model_states',self.initial_target)
		...
		# try:
		# 	self.ts = message_filters.ApproximateTimeSynchronizer([self.joint_state_subscription, self.target_point_subscription], queue_size=50, slop=0.01, allow_headerless=True)
	...
	def action_space(self):
		low=[-3.14159,-1.57079,-2.51327,-3.14159,-3.14159,-3.14159]
		high=[3.14159,1.57079,2.51327,3.14159,3.14159,3.14159]
		return [low,high]



	def initial_target(self,target_point_msg):
	...
	def initial_joint(self, joint_state_msg):
	...
	def get_end_effector_transformation(self):
		...
		try:
			...
			self.robot_x=round (ef_robot_x,3)
			self.robot_y= round(ef_robot_y,3)
			self.robot_z=round(ef_robot_z,3)
	...
	def reset_environment_request(self):
		...
		self.get_end_effector_transformation()
		...
		self.ABSOLUTE_DISTANCE=np.linalg.norm(robot_end_position - target_point_position)
	...
	def generate_action_funct(self):
		...
		angle_j_2 = random.uniform(-1.0, 1.0)	 # values in degrees [  -90,  90]
	...
	def calculate_reward_funct(self):
		...
		try:
			...
			if distance <= 0.01:
				...
				reward_d = -2**(self.ABSOLUTE_DISTANCE/distance)
	...
	def state_space_funct (self):
		...
		try:
			...
			self.get_logger().error(f"Error occurred: {error_message}")  # Ghi lỗi vào log
			return
		else:
			return state
